      ...
      >
              ...
              <div key={item.cartKey} className="flex gap-4 pb-6 border-b border-grey-dark">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex justify-between items-start gap-2">
                    <div>
                      <h3 className="font-display text-base tracking-widest text-ivory">
                        {item.name}
                      </h3>
                      <p className="font-mono text-xs text-grey-light mt-0.5">
                        {item.size} · {item.color}
                      </p>
                      {/* Show size upcharge notice */}
                      {['2X', '3X'].includes(item.size) && (
                        <p className="font-mono text-[10px] text-gold/60 mt-0.5">
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <div className="flex items-center border border-grey-dark">
                      ...
                      >−</button>
                      <span className="w-8 text-center font-mono text-xs text-ivory">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() => updateQty(item.cartKey, item.quantity + 1)}
        ...
        {items.length > 0 && (
          <div className="px-8 py-6 border-t border-gold/10 space-y-4">
            ...
            <p className="font-body text-xs text-grey-light">
